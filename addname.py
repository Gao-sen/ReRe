...
docunames=os.listdir('./test')  # 获取'./test'目录下的所有文件名，存储在docunames列表中，test 可替换

for i in range(len(docunames)):  # 循环遍历docunames列表中的文件名
    with open('test/' + docunames[i], 'r') as f:  # 以只读模式打开'./test'目录下的文件
    ...
    with open(docunames[i], mode="w") as f:  # 以写入模式打开与原文件同名的新文件
        f.write(data_lines)  # 将修改后的文件内容写入新文件中，输出位置与'./test'路径同源